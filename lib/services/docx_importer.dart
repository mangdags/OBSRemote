...
class FighterPresetDocXImporter {
  final String key;
  final String entryName;
  final String w;
  final String wb;

  FighterPresetDocXImporter({
    required this.key,
...
class DocxImporterResult {
  final Map<String, FighterPresetDocXImporter> presets;
...
class DocxImporter {
  ...
  static DocxImporterResult importFromBytes(Uint8List bytes) {
    ...
    final primary =
        zip.files.where((f) => f.name == 'word/document.xml').toList();

    // If not found (rare), scan other word/*.xml as fallback
    final fallback = zip.files
        .where((f) =>
            f.name.startsWith('word/') && f.name.endsWith('.xml') && f.isFile)
        .toList();
    ...
    if (doc == null) {
      throw StateError(
          'No tables found in DOCX (scanned: ${candidates.map((e) => e.name).join(", ")}).');
    }

    // Find all tables (by localName, namespace-safe)
    final tables =
        doc.descendantElements.where((e) => e.name.local == 'tbl').toList();
    if (tables.isEmpty) {
    ...
    // rows (w:tr)
    final rows = firstTable.descendantElements
    ...
    String headerAt(int i) =>
        (i < headerCells.length) ? cellText(headerCells[i]).toUpperCase() : "";

    // Your format header example:
    // [0 blank/#] [1 NAME OF ENTRY] [2 W] [3 WB#] [4 VS] [5 NAME OF ENTRY] [6 W] [7 WB#]
    final looksLikeOldFormat = headerCells.length >= 8 &&
    ...
    final looksLikeNewFormat = headerCells.length >= 9 &&
        headerAt(1).contains("ENTRY") &&
        headerAt(3).contains("WB") &&
        headerAt(4).contains("WT");

    final imported = <String, FighterPresetDocXImporter>{};
    ...
    for (int r = 1; r < rows.length; r++) {
      ...
      void addFighter(String? name, String? w, String? wb) {
        ...
        imported[key] = FighterPresetDocXImporter(
          key: key,
          entryName: n,
          w: (w ?? "").trim(),
          wb: (wb ?? "").trim(),
